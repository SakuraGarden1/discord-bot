...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (item.key === 'anti_addiction') {
      user.addiction = false; user.drinkCount = 0;
      saveUser(userId, user);
    ...
    else if (STACKABLE.includes(item.key) && !['lucky_charm','work_boost'].includes(item.key)) user.inventory[item.key] = (user.inventory[item.key]||0) + 1;

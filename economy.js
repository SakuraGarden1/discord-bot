...
function getJob(level) {
  const available = JOBS.filter(j => level >= j.min && level <= j.max);
  if (available.length === 0) {
    const all = JOBS.filter(j => level >= j.min);
    return all[all.length - 1] || JOBS[0];
  }
...
module.exports = { JOBS, getJob, getAvailableJobs, checkLevelUp, rand, xpForNextLevel, shortNum };
